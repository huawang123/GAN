...
def tr():
    ...
    label = tf.one_hot(spares_label, depth=y_dim, name='label')
    mu, sigma = encode(inputs, label, z_dim = 100, is_training=True, reuse=False)
    # sampling by re-parameterization technique
    eplision = tf.random_normal(tf.shape(mu), 0, 1, dtype=tf.float32)
    z_re = mu + sigma * eplision
    faka_data = decode(z_re, label, is_training=True, reuse=False)
        ...
        with tf.device('/gpu:%d' % gpu):
            ...
            for epoch in range(max_epoch):
                ...
                for step in range(setps):
                    ...
                    ls,summary_str, _, nll, kl  = sess.run([loss, sum, trainer, neg_loglikelihood, KL_divergence], feed_dict={input:x,spares_label:y})
                ...
                [im] = sess.run([faka_data], feed_dict={input:x,spares_label:y})
...
def te():
    ...
    faka_data = decode(z_prior,label, is_training=False, reuse=False)
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        with tf.device('/gpu:%d' % gpu):
            ...
            [im] = sess.run([faka_data], feed_dict={z_prior:z_sample_val,spares_label:y})
