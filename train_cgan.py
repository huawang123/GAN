...
def tr():
    ...
    d_real, d_real_logits, _ = discriminator(inputs, label, is_training=True, reuse=False)
    faka_data = generator(z_prior, label, is_training=True, reuse=False)
    d_fake, d_fake_logits, _ = discriminator(faka_data, label, is_training=True, reuse=True)
        ...
        with tf.device('/gpu:%d' % gpu):
            ...
            for epoch in range(max_epoch):
                ...
                for step in range(setps):
                    ...
                    dl,summary_str, _  = sess.run([dis_loss,d_sum, d_trainer], feed_dict={input:x, spares_label:y, z_prior:z_sample_val})
                    train_writer.add_summary(summary_str, epoch*setps+step)
                    print('[Epoch: %s]  Step: %s  Dis_loss: %s' % (epoch, step, dl))
                    if step%10==0:
                        for j in range(3):
                            gl,summary_str, _ = sess.run([gen_loss,g_sum, g_trainer], feed_dict={spares_label:y, z_prior: z_sample_val})
                            train_writer.add_summary(summary_str, epoch * setps + step)
